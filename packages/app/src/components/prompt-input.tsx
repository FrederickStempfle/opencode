...
import { GoalRainbowBorder, type GoalBorderState } from "./prompt-input/goal-rainbow-border"
...
export type PromptInputControls = {
  ...
  session: {
    ...
    goal?: {
      active: () => boolean
      // Visual state for the goal border: "active" while working, "done" for a brief
      // green flash once achieved, "paused"/"error" when the loop stalls, undefined when
      // nothing should show.
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const handleSlashSelect = (cmd: SlashCommand | undefined) => {
    ...
    if (cmd.type === "custom" || cmd.slashArgs) {
  ...
  const { abort, handleSubmit } =
    ...
    createPromptSubmit({
      ...
      goal: props.controls.session.goal,
    })
      ...
      <Switch>
        <Match when={props.controls.newLayoutDesigns}>
          <div class="flex flex-col gap-3">
            {/* relative, non-clipped wrapper so the goal border's glow can bleed outside
                the form (the form has overflow:clip for its rounded corners). */}
            <div class="relative">
              <DockShellForm
              ...
              <Show when={props.controls.session.goal?.display()}>
                {(state) => <GoalRainbowBorder state={state()} />}
              </Show>
            </div>
            <Show when={newSession() && selectedProject()}>
